...
router.get('/search', [
    query('query')
      .trim()
      .notEmpty().withMessage('El término de búsqueda no puede estar vacío')
...
router.get('/user/:userId', [
    param('userId')
      .trim().notEmpty().withMessage('Usuario no encontrado')
      .isMongoId().withMessage('Usuario no encontrado')
  ],
  handleInputErrors,
...
router.post('/', authenticate, [
    ...
    body('title').isLength({ max: 48 }).withMessage('El titulo tiene un máximo de 48 caracteres'),
    body('description').custom(value => sanitizeInput(value).length > 0).withMessage('La Descripción es obligatoria'),
    body('code').custom(value => sanitizeInput(value).length > 0).withMessage('El código es obligatorio'),
  ],
  handleInputErrors,
...
router.put('/:snippetId', authenticate, [
    body('title').trim().notEmpty().withMessage('El titulo es obligatorio'),
    body('title').isLength({ max: 48 }).withMessage('El titulo tiene un máximo de 48 caracteres'),
    body('description').custom(value => sanitizeInput(value).length > 0).withMessage('La Descripción es obligatoria'),
    body('code').custom(value => sanitizeInput(value).length > 0).withMessage('El código es obligatorio'),
...
router.delete('/:snippetId', authenticate, [
    param('snippetId').isMongoId().withMessage('ID no valido'),
  ],
  handleInputErrors,
  isOwnerOfSnippet,
