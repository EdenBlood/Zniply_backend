...
export class SnippetController {
  static searchSnippets = async (req: Request<{},{},{},{query: string}>, res: Response) => {
  ...
  static getAllUserSnippets = async (req:Request, res:Response) => {
    ...
    try {
      ...
      res.json({msg: "Se cargaron los snippets correctamente", snippet: snippets, user});
    } catch (error) {
      res.status(500).json({ msg: 'Error al Traer los Snippets del usuario'});
    }
  }
  
  static createSnippet = async (req: Request<{},{},TipTapEntries>, res: Response) => {
    ...
    try {
      //* Crea snippet
      const snippet = new Snippet({title, description, code});
  ...
  static getAllSnippets = async (req: Request, res: Response) => {
    try {
      ...
      const snippet = await Snippet.find({user: req.user._id}).sort({ createdAt: -1 });
  ...
  static updateSnippet = async (req: Request<{},{},TipTapEntries>, res: Response) => {
    ...
    try {
      ...
      res.json({ msg: "Snippet updated successfully", snippet: req.snippet });
    } catch (error) {
